
//01 Imprimindo no console;
{
    console.log(1,3,5,7,9);  //declarando valor diretamente;
    console.log(2,4,6,8,10); 
};

/*---------------------------------------*/

//02 Variáveis e constante;
{                                //Variáveis e constantes amarzenam dados;
    var quantidade = 7;          //Variáveis var possuem escopo global (São válidas para todo o código);
    let preco = 3.5;             //Variáveis let possuem escopo em bloco (Só "existem" dentro deste bloco de código);
    let x;  
    const z = 100;               /*Constantes const possuem escopo em bloco e 
                                   não podem ser alteradas ou inicializadas vazias;*/

    console.log(quantidade);     //Imprimindo 
    console.log(preco);
    console.log(x);
    console.log(z);

    preco = 40;                 //Alterando valor de uma váriaverl;

    console.log(preco);
};

/*---------------------------------------*/

//03 Cálculos com variáveis;
{

    let vendidos = 50;
    let valor = 10.90;

    //Cálculo no print;
    console.log(vendidos + valor);

    //Cálculo na variável;
    let soma = vendidos + valor;
    let subtrai = vendidos - valor;
    let multiplica = vendidos * valor;
    let divide = vendidos / valor;
    let equacao = (vendidos + 
                     (valor * vendidos + 10)
                   ) / valor;

    console.log(soma);
    console.log(subtrai);
    console.log(multiplica);
    console.log(divide);
    console.log(equacao);

};

/*---------------------------------------*/

//04 Concatenando textos;
{
    let nome = "Vitor";
    let adjetivo = "normal";
    let numero = 3;

    console.log(nome + " é " + adjetivo);
    console.log("Aluno: " + nome
                + " --- Truma: " + numero);

    //Texto com número;
    console.log("10" + numero);   //+ = Concatena; 
    console.log(nome + numero);   
    console.log("10" * numero);   //(*, / ou -) Calcula como número normalmente;
    console.log(nome - numero);   // Falha;   
};

/*---------------------------------------*/

//05 Tipos de variavél 01;
{
    let valor = 315.895;            //Número;;
    let comida = "macarronada";     //Texto - String;
    let fato = true;                //Verdadeiro ou falso - Boolean;

    console.log(typeof valor);
    console.log(typeof comida);
    console.log(typeof fato);
};

/*--------------EXEMPLOS------------------*/

//Ex01: Cálculando área de uma circunferência;

//01 básica
{    
    let raio = 10;         //Formula: Area = (PI * raio²); 
    const PI = 3.141592;
    let area = (PI * (raio * raio));

    console.log("A área do círculo de raio " + 
                raio + ", é " + area + " m².");
};

//02 com math;
{
    let raio = 10;
    let area = ((Math.PI) * Math.pow(raio, raio));    //Math.PI puxa o valor de pi, Math.pow eleva;

    console.log("A área do círculo de raio " + 
                raio + ", é " + area + " m².");
};

//Ex02: Trocando de valores entre váriaveis;

//01 básica;
{    
    let x = 1;    //Trocar valor de x com y;
    let y = 2;

    console.log(x);
    console.log(y);

    let z = x;
    x = y;
    y = z;

    console.log(x);
    console.log(y);
};

//02 melhor;
{    
    let x = 1;    //Trocar valor de x com y;
    let y = 2;

    [x, y] = [y, x];
    
    console.log(x);
    console.log(y);
};